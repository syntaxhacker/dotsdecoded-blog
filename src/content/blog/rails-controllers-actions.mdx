...
<QuizDemo client:visible questions={[
  {
    question: "In a Rails controller, what is the difference between @variable and a local variable (without @)?",
    options: [
      "There is no difference — both are available in views",
      "Only instance variables (@variable) are passed to the view; local variables stay in the controller",
      "Local variables are faster to access",
      "Instance variables are private; local variables are public"
    ],
    correctIndex: 1,
    points: 10,
    explanation: "Instance variables (starting with @) cross the controller-view boundary automatically. Local variables (without @) do not — they are scoped to the controller action only."
  },
  {
    question: "What happens if a before_action filter calls redirect_to in Rails?",
    options: [
      "The action still runs after the redirect",
      "The action and any remaining filters do not run — the response is sent immediately",
      "Rails raises a DoubleRenderError",
      "The redirect is ignored"
    ],
    correctIndex: 1,
    points: 10,
    explanation: "If a before filter renders or redirects, the action and remaining filters are skipped. The response is sent immediately, preventing the action from executing."
  },
  {
    question: "Why is params.permit! dangerous in production code?",
    options: [
      "It makes the application slower",
      "It permits all submitted parameters, allowing attackers to set fields like admin: true via hidden form fields",
      "It raises an error on every request",
      "It only works with GET requests"
    ],
    correctIndex: 1,
    points: 15,
    explanation: "params.permit! whitelists every parameter, defeating the purpose of strong parameters. An attacker can add hidden fields to set admin: true or modify any attribute."
  },
  {
    question: "When should you use flash.now instead of flash?",
    options: [
      "When you want the message to appear on multiple pages",
      "When you render a template instead of redirecting — flash messages only persist for the next request",
      "When you need to store complex data objects",
      "When the user is not logged in"
    ],
    correctIndex: 1,
    points: 10,
    explanation: "flash stores messages for the next request (works with redirects). flash.now stores messages for the current request only (works with render). Using flash with render causes the message to appear on the following request."
  },
  {
    question: "What is the difference between a member route and a collection route?",
    options: [
      "Member routes use GET; collection routes use POST",
      "Member routes act on a single resource (include :id); collection routes act on all resources (no :id)",
      "Member routes are faster; collection routes are slower",
      "There is no difference"
    ],
    correctIndex: 1,
    points: 10,
    explanation: "Member routes operate on a specific resource and include :id in the URL (like /posts/:id/publish). Collection routes operate on the entire collection without :id (like /posts/search)."
  },
  {
    question: "Why does the POST/Redirect/GET pattern exist?",
    options: [
      "It makes pages load faster",
      "It prevents duplicate form submissions when users refresh after a POST — the redirect causes a GET request instead",
      "It's required by HTTP/2",
      "It prevents CSRF attacks"
    ],
    correctIndex: 1,
    points: 10,
    explanation: "After a successful POST (like creating a record), redirecting causes the browser to make a GET request. Refreshing re-requests the GET page harmlessly instead of resubmitting the form and creating a duplicate."
  },
  {
    question: "What are the three types of parameters that arrive in a Rails controller's params hash?",
    options: [
      "Headers, cookies, and body",
      "URL parameters, query parameters, and form parameters",
      "GET, POST, and PUT",
      "Model, view, and controller parameters"
    ],
    correctIndex: 1,
    points: 10,
    explanation: "URL parameters are embedded in the path (:id), query parameters come after ? in the URL, and form parameters are submitted via POST/PUT/PATCH from HTML forms."
  }
]} />
